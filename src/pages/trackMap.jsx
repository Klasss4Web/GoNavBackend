...
import { Page, Icon, f7, Button } from "framework7-react";

import "../css/map.css";
import MapComponent from "../components/map.jsx";
import NotificationDot from "../components/component";
import { useBusStops } from "../js/mapComponent";
...
import { BUS_INFO_CONSTANTS } from "../constants/busInfoConstants.js";
import { countItemsLeft, getNextItem } from "../utils/nextItemInList.js";
const gpsTopic = "location/";

const TrackMap = (props) => {
  const { selectedTracker, setSelectedTracker, mqttClient, isConnected } =
    useGlobalContext();

  const [currentLocation, setCurrentLocation] = useState(0);

  const topic = selectedTracker?.routeName
    ? `${gpsTopic}${selectedTracker.routeName}`
    : null;
  ...
  useEffect(() => {
    ...
    const getNextBustStop = getNextItem(
      busStopsMap,
      "name",
      gpsData?.busstopName
    );
    const stopsLeft = countItemsLeft(busStopsMap, "name", gpsData?.busstopName);
    ...
    if (gpsData?.busStopFlag && gpsData?.busstopName !== "non") {
      ...
      sessionStorage.setItem(
        BUS_INFO_CONSTANTS.ACTIVE_BUS_STORAGE,
        JSON.stringify({
          stopsLeft,
          getNextBustStop,
