...
import { Button, Icon, Gauge } from "framework7-react";
...
const SwipeableFooter = ({ newSpeed, totalStop, nextStop }) => {
  ...
  // const { device, requestDevice, readBatteryLevel, error } = useBluetooth();
  ...
  const nextBusStop =
    selectedTracker?.getNextBustStop ||
    tripDetails?.getNextBustStop ||
    nextStop;

  const stopsLeft =
    selectedTracker?.stopsLeft || tripDetails?.stopsLeft || totalStop;
            ...
            <div className="align-horizontally">
              <div className="grid grid-cols-3 ">
                ...
                <div
                  ...
                  style={{ justifyContent: "flex-end" }}
                >
                  <span> {nextBusStop || selectedTracker?.busStopName}</span>
                  <span style={{ color: "gray", fontSize: "0.8rem" }}>
                    Next Stop
                  </span>
                </div>
